...
多账号的产品研究整理工具:**邮箱登录 + 管理员建员工账号,登录后全员共享同一份产品研究数据**;
...
│   ├── permissions.py   # 权限谓词(具名函数,不建角色/权限表);首个是 can_delete_comment
...
└── frontend/            # React SPA
    └── src/
        ...
        └── pages/       # HomePage / ProductList / ProductFormPage / ProductDetail /
                         ...
                         # BlogList / BlogDetail / BlogFormPage /
...
- **布局(第四版)**:`AppLayout` 外层 `Layout` 直接放 `Header`(横贯整宽:左品牌、右用户菜单),
  内层再放 `Sider` + `Content`;侧栏是**固定导航**(首页 / 产品分析 / 需求记录 / 博客),
