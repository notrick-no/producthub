...
> 需求来源:产品数据见 `第二版产品-已完成.md`,多账号登录见 `第三版产品-开发中.md`。
> 本文描述当前实现与关键约定,代码有改动时应同步更新。

多账号的产品研究整理工具(第三版):**邮箱登录 + 管理员建员工账号,登录后全员共享同一份产品研究数据**;
后端 REST API + Postgres(服务端会话,HttpOnly cookie),前端 React SPA。开发在本机单机跑
...
│   ├── models.py        # SQLAlchemy 模型(含 users / sessions)
│   ├── schemas.py       # Pydantic 输入输出 Schema(含 Auth / User)
...
│   └── routers/         # auth.py(登录/登出/改密)、users.py(管理员账号管理)、categories.py / products.py
...
└── frontend/            # React SPA
    └── src/
        ├── api/         # client.ts(封装 fetch + 401 回调)+ resources.ts(业务接口)
        ├── auth/        # AuthContext.tsx(AuthProvider / useAuth / RequireUser / RequireAuth / RequireAdmin)
        ├── types.ts     # 与后端 schema 对齐的 TS 类型(snake_case)
        ├── components/  # AppLayout(顶栏头像菜单/登出)/ CategoryManageModal
        └── pages/       # ProductList / ProductFormPage / ProductDetail / Login / AccountsPage / ChangePasswordPage / SetPasswordPage(懒加载)
```
...
核心产品表(三张主表 + 两张随产品的子表)+ 第三版新增的**账号 / 会话**两张表:
...
- 前端路由门禁(`src/auth/AuthContext.tsx`):`RequireAuth`(未登录 → `/login`;登录但
  `must_change_password` → `/change-password`)包着产品页与 `/users`;`/users` 外再套 `RequireAdmin`
